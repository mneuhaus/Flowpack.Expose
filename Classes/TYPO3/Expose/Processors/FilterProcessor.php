...
class FilterProcessor extends AbstractProcessor {
	...
	public function process($query) {
		...
		$this->viewHelperVariableContainer->add('TYPO3\Expose\Processor\FilterProcessor', 'filter', $filter);
		$content = $this->viewHelperVariableContainer->getView()->renderPartial('Filter', NULL, array(
			'filter' => $filter,
			'fields' => $fields
		));
		$this->viewHelperVariableContainer->remove('TYPO3\Expose\Processor\FilterProcessor', 'filter');
